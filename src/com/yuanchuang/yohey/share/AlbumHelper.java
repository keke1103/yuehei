	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				if (bucket == null) {
					...
					bucket.imageList = new ArrayList<ImageBucket.ImageItem>();
					bucket.bucketName = bucketName;
				}
				bucket.count++;
				ImageBucket.ImageItem imageItem = new ImageBucket.ImageItem();
		...
		while (itr.hasNext()) {
			...
			for (int i = 0; i < bucket.imageList.size(); ++i) {
				ImageBucket.ImageItem image = bucket.imageList.get(i);
